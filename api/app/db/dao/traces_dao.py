...
class TraceDAO:
    ...
    @staticmethod
    def _get_all_sessions_stmt(project_uuid: UUID) -> Subquery:
        span_attrs_stmt = (
            select(
                Trace.timestamp,
                Trace.trace_id,
                ...
                literal_column("SpanAttributes['gen_ai.usage.prompt_tokens']").label(
                    'prompt_tokens'
                ),
        ...
        return select(join_stmt, errors_stmt).join(
            errors_stmt, join_stmt.c.session_uuid == errors_stmt.c.session_uuid
        )

    @staticmethod
    def _get_all_root_traces_by_project_uuid_stmt(
        project_uuid: UUID,
        ...
        token_usage = (
            select(
                ...
                func.sum(base_spans.c.total_tokens.cast(Integer)).label('total_tokens'),
            )
            .filter(
        ...
        error_counts = (
            select(
                base_spans.c.service_name,
                base_spans.c.trace_id,
                func.sum(base_spans.c.status_code == 'Error').label('number_of_errors'),
            )
            .group_by(base_spans.c.service_name, base_spans.c.trace_id)
            .subquery()
            .alias('error_count')
        ...
        trace_metrics = (
            select(
                ...
                func.max(base_spans.c.timestamp).cast(String).label('latest_span_ts'),
        ...
        return select(
            ...
            and_(
                ...
                metrics_with_tokens_and_duration.c.trace_id == error_counts.c.trace_id,
            ),
        )

    def get_all_sessions(self, project_uuid: UUID) -> list[RowMapping]:
        with self.db.begin_session() as session:
            stmt = self._get_all_sessions_stmt(project_uuid)
    ...
    def get_all_sessions_paginated(
        ...
        with self.db.begin_session() as session:
            ...
            if sort:
                stmt = (
                    stmt.order_by(asc(text(order_by_column_name(sort))))
                    if order == OrderType.ASC
                    else stmt.order_by(desc(text(order_by_column_name(sort))))
                )
    ...
    def get_all_root_traces_by_project_uuid(
        ...
        with self.db.begin_session() as session:
            stmt = self._get_all_root_traces_by_project_uuid_stmt(
