...
@pytest.fixture
def drift_dataset_demo(spark_fixture, test_data_dir):
    return (
        spark_fixture.read.csv(
...
def test_drift_demo(spark_fixture, drift_dataset_demo):
    output = OutputType(
        prediction=ColumnDefinition(
            name='prediction',
            type=SupportedTypes.float,
    ...
    target = ColumnDefinition(
        name='ground_truth', type=SupportedTypes.float, field_type=FieldTypes.numerical
    )
    timestamp = ColumnDefinition(
        name='timestamp', type=SupportedTypes.datetime, field_type=FieldTypes.datetime
    )
    ...
    features = [
        ColumnDefinition(
            ...
            type=SupportedTypes.float,
            field_type=FieldTypes.numerical,
            drift=drift_ks + drift_ws + drift_js + drift_kl + drift_hl + drift_psi,
        ),
        ColumnDefinition(
            name='sex',
            type=SupportedTypes.float,
    ...
    raw_current_dataset, raw_reference_dataset = drift_dataset_demo
    current_dataset = CurrentDataset(
        model=model, raw_dataframe=raw_current_dataset, prefix_id=prefix_id
    )
    reference_dataset = ReferenceDataset(
    ...
    assert not deepdiff.DeepDiff(
        drift,
        res.test_drift_demo_multiclass,
        ignore_order=True,
        significant_digits=6,
    )
